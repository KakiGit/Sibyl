...
it("should handle complex filename for slug generation", async () => {
    ...
    it("should include llmEnhanced flag in response when using LLM", async () => {
      const { writeFileSync, existsSync, mkdirSync } = await import("fs");
      const { join } = await import("path");
      const { tmpdir } = await import("os");
      const { ingestWithLlm } = await import("../processors/ingest.js");

      await storage.wikiPages.create({
        slug: "existing-concept",
        title: "Existing Concept",
        type: "concept",
        contentPath: join(testWikiDir, "concepts/existing-concept.md"),
        summary: "An existing concept for cross-referencing",
        tags: ["concept"],
      });
      ...
      wikiManager.createPage({
        ...
        type: "concept",
        slug: "existing-concept",
        content: "This is an existing concept in the wiki.",
        tags: ["concept"],
        sourceIds: [],
        createdAt: Date.now(),
        updatedAt: Date.now(),
      ...
      const mockLlmProvider = {
        call: async () => ({
          content: JSON.stringify({
            title: "LLM Generated Page",
            summary: "A page generated by LLM",
            content: "# LLM Generated Page\n\nThis content was generated by LLM with cross-references.\n\nSee [[existing-concept]] for related information.",
            tags: ["llm", "generated"],
            type: "concept",
            crossReferences: ["existing-concept"],
          }),
          model: "test-model",
      ...
      const slug = "llm-generated"
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");

      const tempDir = join(tmpdir(), "sibyl-mcp-test-ingest-llm-gen");
      if (!existsSync(tempDir)) {
        mkdirSync(tempDir, { recursive: true });
      }

      const contentPath = join(tempDir, `${slug}.txt`);
      writeFileSync(contentPath, "Test content for LLM generation");

      const rawResource = await storage.rawResources.create({
        type: "text",
        filename: "llm-generated.txt",
        contentPath,
        metadata: {
          title: "LLM Generated",
          contentLength: 30,
      ...
      const result = await ingestWithLlm({
        ...
        title: "LLM Generated Page",
        wikiFileManager: wikiManager,
        llmProvider: mockLlmProvider as unknown as { call: (sys: string, user: string) => Promise<{ content: string; model: string; usage?: { promptTokens: number; completionTokens: number; totalTokens: number } }> },
        useLlm: true,
      });

      expect(result.wikiPageId).toBeDefined();
      expect(result.slug).toBe("llm-generated-page");
      expect(result.generatedContent.crossReferences).toContain("existing-concept");
