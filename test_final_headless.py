#!/usr/bin/env python
"""Final optimized headless test - uses qwen2.5:0.5b for fast responses."""
...
async def run_server():
    ...
    relevance_evaluator = CachedRelevanceEvaluator(
        embedder=embedder, cache_ttl=600, use_llm=False
    )

    server = IpcServer()
    memory_handler = FastMemoryHandler(simple_store, embedder)
...
async def test_memory():
    ...
    print(
        f"  1.1 Add episode: {elapsed:.2f}s - {resp.get('result', {}).get('episode_id', 'error')[:16]}"
    )
    ...
    resp = await send_ipc(
        "memory.add_episode",
        {
            "content": "Discussed using FalkorDB for graph storage.",
            "source_description": "Tech discussion",
            "session_id": "perf-test",
        },
    )
    elapsed = time.time() - start
    print(f"  1.2 Add episode: {elapsed:.2f}s")

    start = time.time()
    resp = await send_ipc("memory.query", {"query": "qwen model", "num_results": 5})
    elapsed = time.time() - start
    episodes = resp.get("result", {}).get("episodes", [])
    print(f"  1.3 Query memory: {elapsed:.2f}s - {len(episodes)} results")

    start = time.time()
    resp = await send_ipc(
        "memory.get_context", {"query": "database", "session_id": "perf-test"}
    )
    elapsed = time.time() - start
    context_len = len(resp.get("result", {}).get("context", ""))
    print(f"  1.4 Get context: {elapsed:.2f}s - {context_len} chars")
...
async def test_relevance():
    ...
    resp = await send_ipc(
        ...
        {
            "facts": [
                {"content": "qwen2.5:0.5b is fast"},
                {"content": "FalkorDB uses Redis"},
    ...
    print(f"  2.1 Evaluated: {elapsed:.2f}s - {len(results)} relevant")
    for r in results[:2]:
        print(
            f"      [{r.get('score', 0):.2f}] {r.get('fact', {}).get('content', '')[:40]}"
        )
...
async def test_prompt():
    ...
    resp = await send_ipc(
        "prompt.build",
        {
            "user_query": "How to optimize inference?",
            "context": {"project": "Sibyl"},
            "memories": {"context": "Use qwen2.5:0.5b for speed."},
        },
    )
    elapsed = time.time() - start
    prompt = resp.get("result", {}).get("prompt", "")
    print(f"  3.1 Build prompt: {elapsed:.2f}s - {len(prompt)} chars")
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            await send_ipc(
                "memory.add_episode",
                {
                    "content": f"OpenCode session {session_id} started with qwen2.5:0.5b",
            ...
            await send_ipc(
                "memory.add_episode",
                {
                    "content": f"Conversation: {len(messages)} messages exchanged",
            ...
            resp = await send_ipc(
                "memory.query", {"query": "conversation", "session_id": session_id}
            )
            episodes = resp.get("result", {}).get("episodes", [])
            print(f"  4.4 Memory query: {len(episodes)} episodes stored")
...
async def run_tests():
    print("\n" + "=" * 60, flush=True)
    print("Sibyl Final Optimized Headless Test", flush=True)
    print("Models: qwen2.5:0.5b (fast) + all-MiniLM-L6-v2 (embeddings)", flush=True)
    print("=" * 60, flush=True)
