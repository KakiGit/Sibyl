#!/usr/bin/env python
"""Optimized headless test using fakeredis for in-memory testing."""
...
async def run_server():
    print("[SERVER] Using FakeRedis (in-memory)...", flush=True)
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.post(f"{base_url}/session", json={}) as resp:
                if resp.status != 200:
                    ...
                    print(f"    Failed to create session: {resp.status} - {text[:100]}")
                    return True
            ...
            await send_ipc_request(
                "memory.add_episode",
                {
                    "content": f"User asked about math. OpenCode responded with {len(messages)} messages.",
                    "source_description": "Conversation",
                    "session_id": session_id,
                },
            )
            ...
            resp = await send_ipc_request(
                "memory.query",
                {"query": "math", "session_id": session_id},
            )
            episodes = resp.get("result", {}).get("episodes", [])
