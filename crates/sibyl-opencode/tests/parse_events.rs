...
fn test_parse_message_part_updated_text() {
    ...
    match event {
        OpenCodeEvent::MessagePartUpdated { properties } => match properties.part {
...
fn test_parse_real_sse_event() {
    ...
    match event {
        OpenCodeEvent::MessagePartDelta { properties } => {
            assert_eq!(properties.delta, "Hi");
            println!(
                "Successfully parsed delta event with delta: {}",
                properties.delta
            );
        }
        _ => panic!("Expected MessagePartDelta"),
    ...
    match event {
        OpenCodeEvent::MessagePartUpdated { properties } => match properties.part {
            Part::Text { text, time, .. } => {
                ...
                println!(
                    "Successfully parsed message.part.updated with text: {}, end time: {}",
                    text,
                    t.end.unwrap()
                );
            }
